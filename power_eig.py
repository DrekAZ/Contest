...
itimes, max_eig, max_ev = power_eig(max_ev, mat_a, 10, 1.0e-10)
...
itimes, min_eig, min_ev = inv_power_eig(min_ev, mat_a, 10, 1.0e-10)
print('iterative times = ', itimes)
print('min_eig = ', min_eig)
print('min_ev = ', min_ev, '\n')

# 桁数
true_max_eig = slinalg.norm(eigval[0])
true_min_eig = slinalg.norm(eigval[2])
digits_max_eig = np.floor( -spy.log10( np.abs(true_max_eig - max_eig) / true_max_eig ) )
digits_min_eig = np.floor( -spy.log10( np.abs(true_min_eig - min_eig) / true_min_eig ) )
print('max_eig digits = ',digits_max_eig)
print('min_eig digits = ', digits_min_eig)